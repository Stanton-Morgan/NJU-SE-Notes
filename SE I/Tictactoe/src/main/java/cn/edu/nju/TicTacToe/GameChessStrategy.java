...
class AtMost5 extends GameChessStrategy{
	private int[] Xi = new int[5];
	private int[] Xj = new int[5];
	private int[] Oi = new int[5];
	private int[] Oj = new int[5];
	...
	public void putChess(char[][] cells, Player currentPlayer, String chessPos){
		...
		if(currentPlayer == Player.X){
			if(countX >= 5){
				cells[Xi[countX % 5]][Xj[countX % 5]] = '_';
			}
			Xi[countX % 5] = i;
			Xj[countX % 5] = j;
			cells[Xi[countX % 5]][Xj[countX % 5]] = 'X';
			countX++;
		}else{
			if(countO >= 5){
				cells[Oi[countO % 5]][Oj[countO % 5]] = '_';
			}
			Oi[countO % 5] = i;
			Oj[countO % 5] = j;
			cells[Oi[countO % 5]][Oj[countO % 5]] = 'O';
			countO++;
		}
	}
}
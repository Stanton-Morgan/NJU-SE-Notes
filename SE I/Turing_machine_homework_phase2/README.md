...
1. 输入的磁带中出现了不属于输入符号集的字符 Done
2. 图灵机中的磁带数和输入磁带的磁带数冲突 Done
3. 终结状态集不属于状态集 Done
4. 空格符号出现在了输入符号集 Done
5. 空格符号没有出现在磁带符号集 Done
6. 输入符号集并不是磁带符号集的子集 Fone
7. 迁移函数的新旧状态不属于状态集 TODO resolver
8. 迁移函数读取的符号和新写入的符号不属于磁带符号集TODO resolver
